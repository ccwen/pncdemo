...
var textnavigator=React.createClass({displayName: 'textnavigator',
  render:function(){
    (
      React.DOM.div(null, 
        React.DOM.button({className: "btn btn-default"}, "Prev"), 
        React.DOM.button({className: "btn btn-default pullright"}, "Next")
      )
    )
...
var viewer = React.createClass({displayName: 'viewer',
  ...
  renderView:function() {
    var v=this.views[this.state.cur];
  ...
  render: function() {
    return (
      React.DOM.div(null, 
        textnavigator({action: this.props.action}), 
        this.renderView()
      )
    );
  }
