...
var stackview = React.createClass({displayName: 'stackview',
  ...
  renderView:function(v) {
    return textview({action:this.action,text:v.content});
...
var textview = React.createClass({displayName: 'textview',
  ...
  toXML:function(s) {
    ...
    for (var i=0;i<res.tokens.length;i++) {
      ...
      out.push(React.DOM.span( {'data-n':i+1, className:classes, key:"k"+i}, res.tokens[i]));
    }
    return out;
  },
  render: function() {
    return (
      React.DOM.div(null, 
        React.DOM.div( {onMouseUp:this.mouseUp},        
